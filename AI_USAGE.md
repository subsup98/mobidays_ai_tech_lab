...
- SQLite는 초기 PoC, 로컬 검증, 빠른 재현 테스트 용도로만 활용했습니다. 별도 서버 설정 없이 파이프라인과 대시보드 흐름을 빠르게 확인하기에는 유리했지만, 최종 제출 실행 기준은 PostgreSQL로 맞췄습니다.
...
- 실제 LLM 경로로는 Gemini Free Tier를 사용했습니다. 초기 개발 과정에서는 mock extractor로 로컬 검증을 진행했지만, 최종 제출 실행 기준은 Gemini API를 사용하도록 맞췄습니다.
